...
```
On success (HTTP 200), the response contains `query_id` (snake_case) — the ID used for polling.

### Step 2 — Poll for results
```
GET https://api.coactive.ai/api/v1/queries/{query_id}?offset=0&limit=10000
```
Poll every 5 seconds until `response["status"] == "Complete"`.

Possible status values: `Queued`, `Running`, `Stopping`, `Stopped`, `Complete`, `Error`

Results are extracted from: `response["result"]["data"]` — each item is a dict with a `"data"` key containing the row values.
